...
const CustomDecor = () => {
  ...
  const TemplesApi = async () => {
    await getTemplesPagignate(page, "decor", 4)
      .then((response) => {
  ...
  useEffect(() => {
    getTemplesPagignate(page, "decor", 4)
      .then((response) => setTempleData(templeData.concat(response.data.data)))
      .catch((error) => alert(error));
  }, [page]);

  return (
    <div className="Paddings-Top-Contain PTC PTCM">
      ...
      {templeData.length > 0 ? (
        <Row className="text-center mt-5">
          <Col lg="12">
            <div className="btn btn-primary" onClick={() => navigate("decor")}>
              View All
            </div>
          </Col>
        </Row>
      ...
      {/* <Row className="text-center mt-5">
        <Col lg="12">
          <div
            ...
            onClick={() => SeeMore()}
            style={{ display: templeData.length === dataCount ? "none" : "" }}
          >
            View All
          </div>
        </Col>
